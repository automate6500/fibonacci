...
1. Create a new GitHub repo and add the exercise files.

    - Add the `Jenkinsfile` to the root of the repo.
    - Move the `fibonacci.sh` file into a directory named `scripts`.

    - *NOTE: The `fibonacci.sh` file must be placed in a sub-directory named `scripts` for the pipeline to run successfully.*
...
1. Review the pipeline stages for a pipeline run
